...
2. If no argument, scan `.execflow/plans/` for directories containing `execplan.md`. If exactly one match, use that directory name as `<topic-slug>`.
...
Read `.execflow/PLANS.md` in full and audit the ExecPlan against that spec in addition to the criteria below.
...
Rewrite in-place at the same file path (`.execflow/plans/<topic-slug>/execplan.md`). Preserve existing `Progress`, `Surprises & Discoveries`, `Decision Log`, and `Outcomes & Retrospective` content.
...
- Ensure every `.execflow/PLANS.md`-required section exists and is substantive

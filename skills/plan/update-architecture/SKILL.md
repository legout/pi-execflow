...
description: >
  ...
  structure. Use after completing a execflow cycle that implemented an
  ExecPlan.
---
...
- A completed or in-progress ExecPlan at `.execflow/plans/<topic-slug>/execplan.md`.
...
2. If no argument, scan `.execflow/plans/` for the most recent ExecPlan and use that directory name as `<topic-slug>`.

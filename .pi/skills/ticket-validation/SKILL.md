...
description: Validate ticket changes using concrete evidence, exact commands, and explicit gap reporting. Use for ticket validation prompts and review passes that audit proof.
---

# Ticket Validation

Use this skill when determining whether a ticket implementation is actually proven correct.
...
If the task starts from a ticket reference rather than an already-resolved ticket context, first read `../ticket-resolution/SKILL.md` and resolve the ticket system correctly.

For `.beads/` / `br` work, do not assume a file-backed ticket path exists.

---
name: ticket-scope-control
description: Keep changes tightly within ticket scope and prevent opportunistic expansion. Use for any manual ticket prompt that risks widening the diff.
---

# Ticket Scope Control

Use this skill whenever a task could drift beyond the current ticket.
...
1. Implement only what the ticket, plan, or explicit instruction requires.
...
6. Do not mutate tracker status (`tk` / `br`) or repo-root `ticket-flow/` runtime artifacts as an incidental side effect of local prompt work.
...
- renames unrelated to the ticket
...
- did I touch files not required by the ticket?

...
- If multiple ExecPlans exist in `.ticket-flow/plans/`, list them and ask which to split.
...
2. If no topic remains, scan `.ticket-flow/plans/` for directories containing `execplan.md`. If exactly one match, use that directory name as `<topic-slug>`.
...
3. If both exist, inspect `.ticket-flow/AGENTS.md` for a line like:
...
4. If both tracker workspaces exist and no primary tracker can be determined from `.ticket-flow/AGENTS.md`, stop and ask the user to rerun with `--tk` or `--br`.
5. If neither tracker workspace exists, tell the user to run `/ticket-flow-init --tk` or `/ticket-flow-init --br` first and stop.
...
1. Follow the ticketization rules from `skills/ticketize/SKILL.md`.
...
5. Suggest `/ticket-flow`, `/ticket-queue`, or `/ticket-execute-standard <ticket-ref>` next.

## br mode behavior

When the selected tracker is `br`:

1. Follow the issueization rules from `.pi/skills/issueize/SKILL.md`.
...
9. Suggest `/ticket-execute-standard <issue-ref>` or focused `/ticket-*` prompts next.

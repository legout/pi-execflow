...
description: Resolve work items reliably from .tickets/ or .beads/ and connect them to ExecPlans when available. Use before manual ticket implementation, planning, validation, or review.
---

# Ticket Resolution

Use this skill whenever work begins from a ticket reference rather than an already-open file.
...
Support `.beads/` as a portability path, but remember that a `br` workspace is **not** a directory of per-ticket markdown files.

## Resolution order

1. If the input is a valid file path, use it directly and treat it as a file-backed ticket.
...
4. Search `.ticket-flow/plans/` for the best associated plan using:
   - ID match
   - slug/title match
...
- Distinguish a file-backed ticket path from a CLI-backed ticket reference.
- Do not assume `.beads/` contains one file per issue; when it is a `br` workspace, use `br` commands instead of file globbing.
- Report the resolved ticket system explicitly as `tk`, `br`, or `other`.

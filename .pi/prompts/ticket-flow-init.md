---
description: Initialize planning + tracker scaffolding (.ticket-flow/, AGENTS.md, and optional tk/br tracker setup)
argument-hint: "[--tk|--br]"
...
Goals:
1. Create or update `.ticket-flow/AGENTS.md` with planning, tracker, and ExecPlan usage instructions.
2. Create `.ticket-flow/PLANS.md` with the project ExecPlan spec.
3. Create `.ticket-flow/settings.yml` with the project's model-role configuration if it is missing.
4. Create or update the project-root `AGENTS.md` so it references `.ticket-flow/AGENTS.md`.
5. Initialize the selected tracker tool safely:
   - `tk` mode → ensure `.tickets/` exists
   - `br` mode → ensure `.beads/` exists via `br init`
6. If `.pi/prompts/` exists, apply `.ticket-flow/settings.yml` to those prompt frontmatter `model:` and optional `thinking:` fields using the repository's deterministic model-sync command.
...
- For `.ticket-flow/AGENTS.md`:
  - If the file does not exist, create it using the generated block shown below.
  - If the file exists and already contains `<!-- ticket-flow-generated -->` and `<!-- /ticket-flow-generated -->`, replace only that generated block so rerunning `/ticket-flow-init --tk` or `/ticket-flow-init --br` can refresh tracker-specific guidance safely.
...
- For `.ticket-flow/PLANS.md`: create it if missing; if it already exists, leave it untouched (the user may have customized it).
- For `.ticket-flow/settings.yml`: create it if missing; if it already exists, leave it untouched unless the user explicitly asks to regenerate it.
- For tracker setup:
...
- If `.pi/prompts/` exists and `.ticket-flow/settings.yml` exists, run the repository's model-sync step so prompt frontmatter reflects the configured model roles.
...
Write `.ticket-flow/settings.yml` with content equivalent to:
...
Read that file before using `pi-ticket-flow`, `tk`, `br`, or ExecPlans in this repository.
<!-- /ticket-flow -->
```

The marker-based blocks enable safe, idempotent re-runs of `/ticket-flow-init`: only the content between the markers is touched.

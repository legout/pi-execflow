...
4. Read `.ticket-flow/plans/<topic-slug>/execplan.md` in full.
...
At the end:
- if `tk` was selected, suggest `/ticket-flow`, `/ticket-queue`, or `/ticket-execute-standard <ticket-ref>`
- if `br` was selected, suggest `/ticket-execute-standard <issue-ref>` or the focused local `/ticket-*` prompts

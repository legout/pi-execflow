...
6. Ensure `br` is available and the repo has a `.beads/` workspace. If not, stop and suggest `/ticket-flow-init --br`.
...
Suggest running `/ticket-execute-standard <issue-ref>` for the local manual workflow, or the focused `/ticket-*` prompts for a narrower pass.

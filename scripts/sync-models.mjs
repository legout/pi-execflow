...
const settingsPath = existsSync(targetSettingsPath)
	? targetSettingsPath
	: packageSettingsPath;
...
const canonicalSettingsPath = join(
...
function resolvePromptConfigWithFallback(
	promptKey,
	primarySettings,
	fallbackSettings,
) {
...
function syncPromptFile(filePath, settings, fallbackSettings) {
	...
	const { prefix, frontmatter, separator, body } = extractFrontmatter(
		text,
		filePath,
	);
	...
	} = resolvePromptConfigWithFallback(promptKey, settings, fallbackSettings);
...
if (!existsSync(settingsPath)) {
	fail(
		`Missing ${settingsPath}. Run /ef-init first or create the settings file.`,
	);
}

const settings = parseSimpleYaml(readFileSync(settingsPath, "utf8"));
const fallbackSettings =
	existsSync(canonicalSettingsPath) && canonicalSettingsPath !== settingsPath
...
if (promptDirs.length === 0) {
	if (existsSync(targetSettingsPath)) {
		fail(
			`Missing ${projectPromptDir}. Run /ef-init first to scaffold .pi/prompts/.`,
		);
	}
...
process.stdout.write(
	`Updated ${changed.length} of ${results.length - skipped.length} configured prompt files.\n`,
);
for (const result of results) {
	const relativePath = result.file.replace(`${repoRoot}/`, "");
	if (result.skipped) {
		process.stdout.write(
			`- ${relativePath}: skipped | no prompt config and no model frontmatter\n`,
		);
		continue;
	}
	const state = result.changed ? "updated" : "unchanged";
	const thinkingSuffix = result.thinking
		? ` | thinking=${result.thinking}`
		: "";
	process.stdout.write(
		`- ${relativePath}: ${state} | source=${result.source} | model=${result.model}${thinkingSuffix}\n`,
	);
}

...
Suggest `/ef-implement <ticket-ref>` for the validation-only implementation workflow, `/ef-review <ticket-ref>` for independent review with direct follow-up creation, and optional external `/execflow-queue` only if that delegated `tk` workflow is available.

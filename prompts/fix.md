...
2. Read the most recent validation result and, if present, the most recent `/ef-review` verdict from the current conversation.
3. Prefer validation failures and acceptance-criteria gaps as the source of truth. If there are no validation failures but there is a review verdict, interpret `# Findings` as the source of truth.
4. If validation already passed and `# Findings` is absent or exactly `- none`, make no code changes and say no fix was required.
...
- Treat the most recent validation result as authoritative for `/ef-implement`; treat the most recent `/ef-review` findings as authoritative for review-driven fixes unless they clearly contain a false positive.
